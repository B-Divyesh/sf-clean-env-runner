...
const assets = ['/', '/index.html', '/environment-proof.webp', '/environment-proof-mobile.webp', '/favicon.svg', '/apple-touch-icon.png', ...discovered];
const cacheVersion = createHash('sha256').update(html).update(JSON.stringify(assets)).digest('hex').slice(0, 12);
const serviceWorker = `const CACHE = 'clean-env-runner-${cacheVersion}';
const SHELL = ${JSON.stringify(assets)};
self.addEventListener('install', (event) => event.waitUntil(caches.open(CACHE).then((cache) => Promise.all(SHELL.map(async (url) => {
